...
export default function Timer({ 
    ...
    const playerOneTime = splitTime(player1, {withoutHours: true});
      const playerTwoTime = splitTime(player2, {withoutHours: true});


    const prevTeam = useRef(turnFor);
    const [animationClass, setAnimationClass] = useState('');

    useEffect(() => {
        if(isGameOn || isGameOver) setAnimationClass('game-breakpoint');
        if (turnFor !== prevTeam.current) {
            prevTeam.current = turnFor;
            setAnimationClass('turnCompleted');
        }
    }, [turnFor, isGameOn, isGameOver])


    const assignClass = (team) => {
        if(animationClass === 'game-breakpoint') return animationClass;
        if (animationClass === 'turnCompleted' && team === turnFor) return animationClass;
        return '';
    }
